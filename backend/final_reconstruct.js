...
// Revert all previous escapes to get cleaner starting point
content = content.replace(/\\`/g, '`');
content = content.replace(/\\\$\{/g, '${');

const markers = ["content:", "theory:"];
...
for (let i = 0; i < allModuleStarts.length; i++) {
    ...
    // Append code up to the current topicId
    newContent += content.substring(lastIndex, current.index);
    let block = content.substring(current.index, blockEnd);

    // If it's a python module, we use our hardcoded deep dive
    const pythonMod = pythonModules.find(p => p.id === current.id);
    if (pythonMod) {
        // Find title index and replace
        block = block.replace(/title:\s*'[^']*'/, `title: '${pythonMod.title}'`);
        // Find content index and replace
        const contentMarker = "content:";
        const cIdx = block.indexOf(contentMarker);
        if (cIdx !== -1) {
            const prefix = block.substring(0, cIdx + contentMarker.length);
            // Search for end of whatever was there
            const endIdx = block.lastIndexOf(',') > block.lastIndexOf('}') ? block.lastIndexOf(',') : block.lastIndexOf('}');
            // Wait, we need to be more precise for the end of the property
            const suffix = block.substring(block.indexOf('\n', cIdx));
            // Actually, the seeder uses indentation.
            // Let's just create a CLEAN object block for Python modules.
            block = `topicId: '${pythonMod.id}',
                title: '${pythonMod.title}',
                content: JSON.stringify(${JSON.stringify(pythonMod.content)})
            `;
        }
    } else {
        // For other modules, we just ENSURE any backtick strings are converted to JSON.stringify if they aren't already.
        markers.forEach(marker => {
            const mIdx = block.indexOf(marker);
            if (mIdx === -1) return;

            const valueStartOffset = mIdx + marker.length;
            const remaining = block.substring(valueStartOffset);

            const templateStart = remaining.indexOf('`');
            const quoteStart = remaining.indexOf('"');
            const jsonStart = remaining.indexOf('JSON.stringify(');
            ...
            // Find ending of this specific property value.
            // property: value, \n
            let valEnd = -1;
            if (type === "template") valEnd = remaining.lastIndexOf('`');
            else if (type === "json") valEnd = remaining.lastIndexOf(')');
            else if (type === "quote") valEnd = remaining.lastIndexOf('"');

            if (valEnd === -1 || valEnd <= valStart) return;

            // Extract what's inside
            const inner = remaining.substring(valStart + 1, valEnd);

            const prefix = block.substring(0, valueStartOffset + valStart);
            const suffix = block.substring(valueStartOffset + valEnd + 1);

            // Reconstruct as JSON stringify
            // If it was already JSON.stringify, we might have accidentally extracted just the argument.
            // But JSON.stringify(JSON.stringify(...)) is double-stringified which is WRONG.
            // So we check.
            if (type === "json") {
                // Leave it as is.
                return;
            ...
            const normalized = "JSON.stringify(" + JSON.stringify(inner) + ")";
            block = prefix + normalized + suffix;
        });
    }
...
console.log("File reconstructed and Python modules finalized.");
