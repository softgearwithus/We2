...
// The 8 Deep Dive Python Modules
const pythonModules = [
    {
...
// Revert backslashes to get raw content for other tracks
content = content.replace(/\\`/g, '`');
content = content.replace(/\\\$\{/g, '${');

const startTag = "const contents = [";
const endTag = "];";
const preamble = content.substring(0, content.indexOf(startTag) + startTag.length);
const mid = content.substring(content.indexOf(startTag) + startTag.length, content.lastIndexOf(endTag));
const postamble = content.substring(content.lastIndexOf(endTag));

// Find all topicIds and their blocks
const idRegex = /topicId:\s*'([^']+)'/g;
const modules = [];
let match;
while ((match = idRegex.exec(mid)) !== null) {
    modules.push({ id: match[1], index: match.index });
}
...
for (let i = 0; i < modules.length; i++) {
    ...
    const blockEnd = next ? next.index : mid.lastIndexOf('}');
    let block = mid.substring(current.index, blockEnd + 1);

    // Deep Dive Python
    const py = pythonModules.find(p => p.id === current.id);
    if (py) {
        cleanedModules.push({
            topicId: py.id,
            title: py.title,
    ...
    // For others, extract title and content
    const titleMatch = block.match(/title:\s*['"]([^'"]+)['"]/);
    const title = titleMatch ? titleMatch[1] : "";

    // Extract content/theory/desc
    const contentTagMatch = block.match(/(content|theory|desc):\s*/);
    let extractedContent = "";
    if (contentTagMatch) {
        const tag = contentTagMatch[0];
        const startIdx = block.indexOf(tag) + tag.length;
        const remaining = block.substring(startIdx);

        // It might be template literal, quoted string, or JSON.stringify(...)
        const templateStart = remaining.indexOf('`');
        const quoteStart = remaining.indexOf('"');
        const jsonStart = remaining.indexOf('JSON.stringify(');

        if (templateStart !== -1 && (jsonStart === -1 || templateStart < jsonStart)) {
            const end = remaining.lastIndexOf('`');
            extractedContent = remaining.substring(templateStart + 1, end);
        } else if (jsonStart !== -1) {
            const argStart = remaining.indexOf('(') + 1;
            const argEnd = remaining.lastIndexOf(')');
            const arg = remaining.substring(argStart, argEnd);
            try {
                extractedContent = eval(arg);
            } catch (e) {
                extractedContent = "{}"; // fallback
            }
        } else if (quoteStart !== -1) {
            const end = remaining.lastIndexOf('"');
            extractedContent = remaining.substring(quoteStart + 1, end);
        }
    }
...
// Generate the new mid section
let newMid = "\n";
cleanedModules.forEach((m, idx) => {
    newMid += `            {
                topicId: '${m.topicId}',
                title: '${m.title}',
                content: JSON.stringify(${JSON.stringify(m.content)})
            }${idx < cleanedModules.length - 1 ? ',' : ''}\n`;
});
