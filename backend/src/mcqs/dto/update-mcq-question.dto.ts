...
export class UpdateMcqQuestionDto {
    ...
    @ApiProperty({ example: 'english', description: 'Subject key or company name', required: false })
    @IsOptional()
    @IsString()
    group?: string;
    ...
    @ApiProperty({ example: ['She is going to school.', 'She are going to school.'], required: false })
    @IsOptional()
    @IsArray()
    @ArrayMinSize(2)
    options?: string[];

    @ApiProperty({ example: 0, description: 'Zero-based index of correct option', required: false })
    @IsOptional()
    @IsInt()
    @Min(0)
