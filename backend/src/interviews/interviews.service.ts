...
export class InterviewsService {
  ...
  async create(dto: CreateInterviewDto): Promise<InterviewSession> {
    ...
    await this.assertProAccess(dto.userId);
  ...
  async findByUser(userId: string): Promise<InterviewSession[]> {
    await this.assertProAccess(userId);
  ...
  private async assertProAccess(userId: string): Promise<User> {
    ...
    if (!isPro) {
      throw new ForbiddenException(
        'Pro subscription required for interview features.',
      );
    }
  ...
  async checkAndIncrementLimit(
    ...
    const user = await this.assertProAccess(userId);

    const now = new Date();
    const lastReset = user.usageLastReset
      ? new Date(user.usageLastReset)
      : null;
    ...
    const currentUsage =
      type === 'audio' ? user.audioDrillUsage : user.videoInterviewUsage;
  ...
  async deductCredit(userId: string, type: 'audio' | 'video') {
    const { allowed, limit, usage } = await this.checkAndIncrementLimit(
      userId,
      type,
    );
    if (!allowed) {
      throw new BadRequestException(
        `Monthly ${type} limit exhausted (${usage}/${limit}). Please upgrade your plan for more.`,
      );
    }
  ...
  async generateAudioDrill(userId: string, topic: string) {
    ...
    if (user.drillTopicsRefreshCount >= 20) {
      return this.geminiService.generateDrillContent(
        'Common Interview Questions',
        { userId, isFallback: true },
      );
    }
  ...
  async analyzeAudioDrill(
    ...
    if (!allowed) {
      throw new BadRequestException(
        `Monthly audio drill limit exhausted (${usage}/${limit}). Please upgrade your plan for more.`,
      );
    }

    // Analyze
    const analysis = await this.geminiService.analyzeAudio(
      audioBase64,
      context,
    );
  ...
  async submitCommunicationSession(
    userId: string,
    theme?: string,
  ): Promise<InterviewSession> {
    await this.assertProAccess(userId);
  ...
  async cancelCommunicationSession(
    userId: string,
    theme?: string,
  ): Promise<InterviewSession> {
    await this.assertProAccess(userId);
  ...
  async performBackgroundAnalysis(
    ...
    console.log(
      `[Service] Starting background analysis for session ${sessionId}`,
    );
    console.log(
      `[Service] Files received: ${files.map((f) => f.fieldname).join(', ')}`,
    );
    ...
    try {
      ...
      if (metadata.reading) {
        for (let i = 0; i < metadata.reading.length; i++) {
          try {
            ...
            if (file) {
              console.log(
                `[Service] Analyzing reading_${i}, buffer size: ${file.buffer.length} bytes`,
              );
              if (file.buffer.length < 1000) {
                // Very rough silence/empty check (1KB)
                console.warn(
                  `[Service] reading_${i} buffer too small, skipping AI analysis.`,
                );
                continue;
              }
              ...
              const analysis = await this.geminiService.analyzeAudio(
                audioBase64,
                `Reading Task: Read this text clearly: "${text}"`,
              );
              if (analysis) {
                console.log(
                  `[Service] reading_${i} score: ${analysis.overallScore}`,
                );
                ...
                if (analysis.actionableCoaching)
                  allCoaching.push(...analysis.actionableCoaching);
              }
            } else {
      ...
      if (metadata.listening) {
        for (let i = 0; i < metadata.listening.length; i++) {
          try {
            ...
            if (file) {
              console.log(
                `[Service] Analyzing listening_${i}, buffer size: ${file.buffer.length} bytes`,
              );
              if (file.buffer.length < 1000) {
                console.warn(
                  `[Service] listening_${i} buffer too small, skipping.`,
                );
                continue;
              }
              ...
              const analysis = await this.geminiService.analyzeAudio(
                audioBase64,
                `Listening Task: Repeat this sentence exactly: "${text}"`,
              );
              if (analysis) {
                console.log(
                  `[Service] listening_${i} score: ${analysis.overallScore}`,
                );
      ...
      if (metadata.extempore) {
        try {
          ...
          if (file) {
            console.log(
              `[Service] Analyzing extempore, buffer size: ${file.buffer.length} bytes`,
            );
            if (file.buffer.length < 1000) {
              ...
              const analysis = await this.geminiService.analyzeAudio(
                audioBase64,
                `Extempore Task: Speak for 60 seconds on: "${topic}"`,
              );
              if (analysis) {
                console.log(
                  `[Service] extempore score: ${analysis.overallScore}`,
                );
                ...
                if (analysis.actionableCoaching)
                  allCoaching.push(...analysis.actionableCoaching);
                if (analysis.communicationPersona)
                  mainPersona = analysis.communicationPersona;
              }
            }
          } else {
      ...
      if (metadata.technical && Array.isArray(metadata.technical)) {
        for (let i = 0; i < metadata.technical.length; i++) {
          try {
            ...
            if (file) {
              console.log(
                `[Service] Analyzing technical_${i}, buffer size: ${file.buffer.length} bytes`,
              );
              if (file.buffer.length < 1000) {
                console.warn(
                  `[Service] technical_${i} buffer too small, skipping.`,
                );
                continue;
              }
              ...
              const analysis = await this.geminiService.analyzeAudio(
                audioBase64,
                `Technical Interview Question: Answer this software engineering question out loud thoughtfully: "${topic}"`,
              );
              if (analysis) {
                console.log(
                  `[Service] technical_${i} score: ${analysis.overallScore}`,
                );
                ...
                if (analysis.actionableCoaching)
                  allCoaching.push(...analysis.actionableCoaching);
                if (
                  analysis.communicationPersona &&
                  mainPersona === 'Assessment Pending'
                )
                  mainPersona = analysis.communicationPersona;
              }
            } else {
      ...
      const finalScore =
        itemsCount > 0 ? Math.round(totalScoreSum / itemsCount) : null;
      results.overallScore = finalScore;

      const session = await this.interviewsRepo.findOne({
        where: { id: sessionId },
      });
      if (session) {
        ...
        console.log(
          `[Service] Completed analysis for session ${sessionId} with ${itemsCount} items.`,
        );
      }
    } catch (error) {
      console.error(
        `[Service] Background analysis failed for session ${sessionId}`,
        error,
      );
      analysisFailed = true;
    } finally {
      if (analysisFailed) {
        ...
        if (session) {
          ...
          session.completedAt = new Date();
          session.feedback =
            'Drill submitted, but analysis failed. Please retry.';
          session.analysis = {
            reading: [],
            listening: [],
            extempore: null,
