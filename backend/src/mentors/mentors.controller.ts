...
export class MentorsController {
    ...
    @Get('mentors')
    @Roles(UserRole.STUDENT, UserRole.COLLEGE_ADMIN, UserRole.MENTOR, UserRole.COMPANY_ADMIN, UserRole.SUPER_ADMIN)
    ...
    async toggleMentor(@Param('id') id: string, @Body() body: { isActive: boolean }) {
    ...
    async createSession(@Request() req: any, @Body() payload: VerifyMentorPaymentDto) {
    ...
    async acceptRequest(@Request() req: any, @Param('id') id: string, @Body() body: { meetingLink: string }) {
    ...
    async logPayout(@Body() body: { mentorId: string; amountInr: number; referenceId: string }) {
        return this.mentorsService.recordPayout(body.mentorId, body.amountInr, body.referenceId);
    }
