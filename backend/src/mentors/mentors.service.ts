...
export class MentorsService {
  ...
  async approveApplication(applicationId: string) {
    ...
    if (!user) {
      throw new NotFoundException(
        'User account not found for this application email',
      );
    }
  ...
  async createSessionFromPayment(
    studentId: string,
    payload: VerifyMentorPaymentDto,
  ) {
    const mentor = await this.mentorRepo.findOne({
      where: { id: payload.mentorId, isActive: true },
    });
    if (!mentor) throw new NotFoundException('Mentor not found');

    const now = Date.now();
    const expectedAmountInPaise =
      mentor.pricePerMinute * payload.durationMinutes * 100;
    ...
    return this.sessionRepo.manager.transaction(async (manager) => {
      ...
      if (!order) {
        throw new BadRequestException(
          'Payment order not found. Please create a new order.',
        );
      }
      ...
      if (order.studentId !== studentId || order.mentorId !== mentor.id) {
        throw new BadRequestException(
          'Payment order does not belong to this booking request.',
        );
      }
  ...
  async createPaymentOrder(
    studentId: string,
    payload: CreateMentorPaymentOrderDto,
  ) {
  ...
  async listAllSessions() {
    ...
    const mentors = mentorIds.length
      ? await this.mentorRepo.findBy({ id: In(mentorIds) })
      : [];
    const users = studentIds.length
      ? await this.usersService.findByIds(studentIds)
      : [];
  ...
  async listMentorRequests(mentorUserId: string) {
    const mentorProfile = await this.mentorRepo.findOne({
      where: { userId: mentorUserId },
    });
    if (!mentorProfile) throw new NotFoundException('Mentor profile not found');
    return this.sessionRepo.find({
      where: { mentorId: mentorProfile.id, status: 'requested' },
      order: { createdAt: 'DESC' },
    });
  }
  ...
  async acceptRequest(
    ...
    const session = await this.sessionRepo.findOne({
      where: { id: sessionId, mentorId: mentorProfile.id },
    });
