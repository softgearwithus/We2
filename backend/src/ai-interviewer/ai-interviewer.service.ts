...
export class AiInterviewerService {
  ...
  async getReport(sessionId: string, userId: string) {
    ...
    if (aiBase && aiKey && session.externalSessionId) {
      const res = await fetch(
        `${aiBase}/ai-interview/sessions/${session.externalSessionId}/report`,
        {
      ...
      if (res && res.ok) {
        ...
        if (data) {
          ...
          try {
            ...
            const upstream = await this.sessionsRepo.findOne({
            ...
            if (upstream) {
              ...
              const existing = await this.moderationRepo.count({
                where: { sessionId: session.id },
              });
              if (!existing) {
                const upstreamEvents = await this.moderationRepo.find({
                  where: { sessionId: upstream.id },
                });
          ...
          const created = this.reportsRepo.create({
            ...
            dimensionScores:
              data.dimension_scores ?? data.dimensionScores ?? null,
  ...
  async startSession(sessionId: string, userId: string) {
    ...
    if (aiBase && aiKey && session.externalSessionId) {
      await fetch(
        `${aiBase}/ai-interview/sessions/${session.externalSessionId}/start`,
        {
          method: 'POST',
          headers: {
