...
export class CollegesService {
    ...
    async listStaff(collegeId: string) {
        return this.staffRepo.find({ where: { collegeId, isActive: true }, order: { createdAt: 'DESC' } });
    }

    private generateCredentialId(collegeCode: string, role: string) {
        const suffix = role === UserRole.COLLEGE_ADMIN ? 'ADM' : role === UserRole.MENTOR ? 'MTR' : 'HOD';
    ...
    async addStaff(collegeId: string, payload: CreateStaffDto) {
        ...
        const existingUser = await this.userRepo.findOne({ where: { email: saved.email } });
        if (existingUser) {
    ...
    async removeStaff(collegeId: string, staffId: string) {
        const staff = await this.staffRepo.findOne({ where: { id: staffId, collegeId } });
    ...
    async listCohorts(collegeId: string) {
        return this.cohortRepo.find({ where: { collegeId }, order: { createdAt: 'DESC' } });
    }
    ...
    async getInstituteDashboard(collegeId: string) {
        ...
        const placedStudents = await this.studentRepo.count({ where: { collegeId, status: 'Placed' } });
    ...
    private async getDepartmentStats(collegeId: string) {
        ...
        return departments.map((dept) => {
            ...
            const avgReadiness = count ? deptStudents.reduce((acc, s) => acc + s.placementReadiness, 0) / count : 0;
    ...
    async getInstituteStudents(collegeId: string, filters: { year?: number; department?: string; status?: string; search?: string; scopeDepartment?: string | null; scopeYear?: string | null }) {
        const query = this.studentRepo.createQueryBuilder('student').where('student.collegeId = :collegeId', { collegeId });
        if (filters.scopeDepartment) {
            query.andWhere('student.department = :scopeDepartment', { scopeDepartment: filters.scopeDepartment });
        }
        if (filters.scopeYear) {
            query.andWhere('student.year = :scopeYear', { scopeYear: Number(filters.scopeYear) });
        }
        if (filters.year) {
    ...
    async getInstitutePlacements(collegeId: string) {
        ...
        const avgCoding = students.reduce((acc, s) => acc + (s.skills?.coding || 0), 0) / total;
        const avgAptitude = students.reduce((acc, s) => acc + (s.skills?.aptitude || 0), 0) / total;
        const avgCommunication = students.reduce((acc, s) => acc + (s.skills?.communication || 0), 0) / total;
        const avgCore = students.reduce((acc, s) => acc + (s.skills?.core || 0), 0) / total;
        return {
            readiness: {
    ...
    async getInstituteReports(collegeId: string) {
        ...
        const deptPerformance = departments.map((dept) => {
            const deptStudents = students.filter((s) => s.department === dept);
            const avg = deptStudents.reduce((acc, s) => acc + s.placementReadiness, 0) / (deptStudents.length || 1);
