...
export class MockTestSection {
    ...
    @Column({ type: 'int', nullable: true, comment: 'Strict timer for this section in minutes, if null it uses global timer' })
    durationMinutes: number;

    @Column({ type: 'int', default: 0 })
    order: number;

    @OneToMany(() => MockTestQuestion, question => question.section, { cascade: true })
