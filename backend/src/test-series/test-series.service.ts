...
import {
  MockTestQuestion,
  MockQuestionType,
} from './entities/mock-test-question.entity';
...
export class TestSeriesService {
  ...
  async getMockTestsByCompany(companyId: string) {
    return this.mockTestRepository.find({
      where: { companyId },
      order: { order: 'ASC', createdAt: 'ASC' },
    });
  }

  async createMockTest(dto: {
    companyId: string;
    title: string;
    description?: string;
  ...
  async updateSection(
    id: string,
    dto: { title?: string; durationMinutes?: number; order?: number },
  ) {
  ...
  async submitTest(
    ...
    if (sectionIds.length > 0) {
      ...
      actualTotalMarks = allQuestions.reduce(
        (sum, q) => sum + (q.marks || 1),
        0,
      );
    }
    ...
    for (const r of payload.responses) {
      ...
      if (question.questionType === MockQuestionType.SINGLE_CORRECT) {
        if (
          question.correctAnswer &&
          r.responseValue === question.correctAnswer
        ) {
        ...
        question.questionType === MockQuestionType.TEXT ||
        question.questionType === MockQuestionType.CODE
      ) {
    ...
    if (requiresAiEvaluation) {
      ...
      this.testEvaluationService
        .evaluatePendingResponses(result.id)
        .catch((err) => {
  ...
  async getResultFull(resultId: string, userId: string) {
    ...
    if (!result)
      throw new NotFoundException('Result not found or access denied');

    if (result.resultType === 'subject_practice') {
      const mcqIds = result.responses
        .map((r) => r.subjectMcqId)
        .filter((id) => id);
      if (mcqIds.length > 0) {
        const mcqs = await this.mcqQuestionRepository.find({
          where: { id: In(mcqIds) },
        });
        result.responses.forEach((r) => {
          ...
          if (matched) {
            ...
            (r as any).question = {
              ...
              correctAnswer:
                matched.correctOptionIndex !== null
                  ? String(matched.correctOptionIndex)
                  : null,
