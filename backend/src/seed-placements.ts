...
import { Placement, PlacementStatus, PlacementType } from './placements/entities/placement.entity';
import { Repository } from 'typeorm';
import { getRepositoryToken } from '@nestjs/typeorm';

async function bootstrap() {
    const app = await NestFactory.createApplicationContext(AppModule);
    const placementRepo = app.get<Repository<Placement>>(getRepositoryToken(Placement));
