...
export class ApplicationsService {
    ...
    async findByPlacement(placementId: string, companyId: string) {
        ...
        const placement = await this.placementsRepository.findOne({ where: { id: placementId } });
        if (!placement) throw new NotFoundException('Drive not found');
        if (placement.companyId !== companyId) throw new ForbiddenException('Access denied. You do not own this drive.');
