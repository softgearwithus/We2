...
import { ProjectLabSubmission, ProjectLabSubmissionStatus } from './entities/project-lab-submission.entity';
...
export class ProjectLabsService {
    ...
    async getMyProgress(userId: string) {
        ...
        const completed = submissions.filter((s) => s.status === ProjectLabSubmissionStatus.COMPLETED || s.status === ProjectLabSubmissionStatus.APPROVED);
        return {
    ...
    async updateSubmission(id: string, dto: UpdateProjectLabSubmissionDto) {
        ...
        if (dto.status === ProjectLabSubmissionStatus.APPROVED && !submission.completedAt) {
            submission.completedAt = new Date();
        }
        if (dto.status === ProjectLabSubmissionStatus.REJECTED) {
            submission.reviewedAt = new Date();
        }
        if (dto.status === ProjectLabSubmissionStatus.APPROVED || dto.status === ProjectLabSubmissionStatus.COMPLETED) {
