...
const loadMockProblems = async () => (await import('./problems.mock')).problems;
...
export const fetchProblems = async (): Promise<Problem[]> => {
    try {
        ...
        return loadMockProblems(); // Fallback to mock data
    }
};
...
export const fetchProblemBySlug = async (slug: string): Promise<Problem | null> => {
    try {
        ...
        const mocks = await loadMockProblems();
        return mocks.find(p => p.id === slug) || null;
    }
};
