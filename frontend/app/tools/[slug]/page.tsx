...
// Generate static parameters for all 'tools' pages
export function generateStaticParams() {
  const toolsPages = getAllSeoPages().filter(page => page.category === 'tools');
  return toolsPages.map((page) => ({
    slug: page.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!pageData || pageData.category !== 'tools') {
  ...
  return {
    ...
    alternates: {
      canonical: `https://emble.in/tools/${slug}`,
    },
    openGraph: {
      title: pageData.title,
      description: pageData.subDescription,
      url: `https://emble.in/tools/${slug}`,
      siteName: 'Emble',
      images: [
        {
...
export default async function ToolsLandingPage({ params }: Props) {
  const { slug } = await params;
  const pageData = getSeoPageBySlug(slug);
