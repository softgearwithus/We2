...
import type { DocArticle } from "../data";
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
              ...
              <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
                ...
                <div className="max-w-none space-y-6">
                  <ReactMarkdown
                    components={{
                      h1: ({ node, ...props }) => (
                        <h1 className="text-4xl md:text-5xl font-black text-brand-black mt-16 mb-8 leading-tight tracking-tight" {...props} />
                      ),
                      h2: ({ node, ...props }) => (
                        <h2 className="text-3xl font-extrabold text-brand-black mt-12 mb-6 tracking-tight flex items-center gap-3">
                          <span className="w-1.5 h-6 bg-brand-orange rounded-full"></span>
                          {props.children}
                        </h2>
                      ),
                      h3: ({ node, ...props }) => (
                        <h3 className="text-2xl font-bold text-brand-black mt-8 mb-4 tracking-tight" {...props} />
                      ),
                      p: ({ node, ...props }) => (
                        <p className="text-lg text-gray-600 leading-relaxed font-medium mb-6" {...props} />
                      ),
                      ul: ({ node, ...props }) => <ul className="space-y-4 my-6 ml-2" {...props} />,
                      ol: ({ node, ...props }) => (
                        <ol className="list-decimal list-outside space-y-4 my-6 ml-6 text-gray-600 font-medium" {...props} />
                      ),
                      li: ({ node, ...props }) => (
                      ...
                      strong: ({ node, ...props }) => (
                        <strong className="text-brand-black font-extrabold bg-brand-orange/5 px-1 rounded" {...props} />
                      ),
                      a: ({ node, ...props }) => (
                        <a className="font-bold text-brand-orange hover:text-brand-orange-hover hover:underline transition-all inline-flex items-center gap-1" {...props} />
                      ),
                      blockquote: ({ node, ...props }) => (
                        <blockquote className="border-l-4 border-brand-orange/30 pl-6 py-2 my-8 italic text-gray-500 bg-gray-50/50 rounded-r-2xl" {...props} />
                      ),
                    }}
                  >
            ...
            <div className="lg:col-span-4 sticky top-32 space-y-8">
              ...
              <Link
                href="/docs"
                className="inline-flex items-center gap-2 text-sm font-bold text-gray-500 hover:text-brand-orange transition-colors"
              >
                <ArrowLeft size={16} />
                Back to Docs Hub
              </Link>
