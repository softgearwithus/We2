...
// Generate static parameters for all 'prep' pages
export function generateStaticParams() {
  const prepPages = getAllSeoPages().filter(page => page.category === 'prep');
  return prepPages.map((page) => ({
    slug: page.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!pageData || pageData.category !== 'prep') {
  ...
  return {
    ...
    alternates: {
      canonical: `https://emble.in/prep/${slug}`,
    },
    openGraph: {
      title: pageData.title,
      description: pageData.subDescription,
      url: `https://emble.in/prep/${slug}`,
      siteName: 'Emble',
      images: [
        {
...
export default async function PrepLandingPage({ params }: Props) {
  const { slug } = await params;
  const pageData = getSeoPageBySlug(slug);
