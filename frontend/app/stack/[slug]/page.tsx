...
export function generateStaticParams() {
  const stackPages = getAllSeoPages().filter(page => page.category === 'stack');
  return stackPages.map((page) => ({
    slug: page.slug,
  }));
}
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!pageData || pageData.category !== 'stack') {
  ...
  return {
    ...
    alternates: {
      canonical: `https://emble.in/stack/${slug}`,
    },
    openGraph: {
      title: pageData.title,
      description: pageData.subDescription,
      url: `https://emble.in/stack/${slug}`,
      siteName: 'Emble',
      images: [
        {
...
export default async function StackSEOPage({ params }: Props) {
  const { slug } = await params;
  const pageData = getSeoPageBySlug(slug);
