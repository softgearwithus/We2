...
export default function MarketRadarPage() {
    ...
    if (isLoading) {
        return (
            <div className="max-w-full max-w-[1400px] mx-auto min-h-[calc(100vh-8rem)] flex items-center justify-center">
    ...
    if (error || !data) {
        return (
            <div className="max-w-full max-w-[1400px] mx-auto min-h-[calc(100vh-8rem)] flex flex-col items-center justify-center text-center px-6">
    ...
    return (
        <div className="max-w-full max-w-[1400px] mx-auto min-h-[calc(100vh-8rem)] flex flex-col pt-6 font-sans pb-12">
            {/* Header */}
            <header className="flex items-center justify-between mb-8 shrink-0">
                <div className="flex items-center gap-3">
                    <div className="w-10 h-10 bg-slate-800 rounded-xl flex items-center justify-center shadow-md shadow-slate-200">
                ...
                <div className="lg:col-span-8 space-y-8">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-sm flex flex-col items-center text-center">
                            <Briefcase className="w-6 h-6 text-slate-700 mb-2" />
                    ...
                    <div className="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 p-6 overflow-hidden">
                        ...
                        <div className="space-y-4">
                            {jobTrends.map((job) => (
                                <div key={job.id} className="group relative flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-2xl border border-slate-200 hover:border-slate-300 hover:shadow-md transition-all bg-slate-50/50">
                                    <div className="flex items-center gap-4 mb-4 sm:mb-0">
                                        <div className={`w-12 h-12 rounded-xl flex items-center justify-center ${job.color} font-bold text-xl`}>
                                            {job.role.charAt(0)}
                                        </div>
                                        <div>
                                            <h3 className="font-bold text-slate-900 group-hover:text-slate-800 transition-colors">{job.role}</h3>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        {insights.map((insight) => (
                            <div key={insight.id} className="bg-slate-900 rounded-3xl p-6 relative overflow-hidden group">
                                <div className="absolute top-0 right-0 w-32 h-32 bg-white/5 rounded-full blur-2xl -mr-10 -mt-10 group-hover:bg-slate-500/10 transition-colors" />
                ...
                <div className="lg:col-span-4 space-y-8">
                    ...
                    <div className="bg-white rounded-3xl border border-slate-200 shadow-xl shadow-slate-200/50 p-6">
                        <h2 className="text-lg font-black text-slate-900 flex items-center gap-2 mb-6">
                            <Code2 className="w-5 h-5 text-slate-700" /> Trending Languages
                        </h2>
                    ...
                    <div className="bg-gradient-to-br from-slate-50 to-blue-50 rounded-3xl border border-slate-200 shadow-xl shadow-slate-200 p-6">
                        <h2 className="text-lg font-black text-slate-900 flex items-center gap-2 mb-6">
                            <Sparkles className="w-5 h-5 text-slate-700" /> Action Items
                        </h2>
                        <p className="text-sm font-medium text-slate-800/70 mb-6">
                        ...
                        <div className="space-y-4">
                            {enhancements.map((enh) => (
                                <div key={enh.id} className="bg-white p-4 rounded-xl shadow-sm border border-white/50 relative overflow-hidden group">
                                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-slate-500 rounded-l-xl opacity-0 group-hover:opacity-100 transition-opacity" />
                                    <div className="flex items-center gap-2 mb-2">
                                        <span className="text-[10px] font-black uppercase tracking-widest text-slate-700 bg-slate-50 px-2 py-0.5 rounded-md">
                                            {enh.category}
                                        </span>
