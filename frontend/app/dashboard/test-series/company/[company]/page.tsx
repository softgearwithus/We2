...
export default function CompanyMcqsPage() {
    ...
    useEffect(() => {
        ...
        setLoading(true);
        fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3001'}/mcqs?category=company&groupKey=${company}&page=${page}&limit=${limit}&order=latest`, {
    ...
    const displayLabel = items[0]?.groupLabel || company.replace(/_/g, ' ');
    ...
    return (
        <div className="min-h-screen font-sans text-slate-900 selection:bg-orange-100 selection:text-orange-700 bg-[#F8FAFC] pb-24">
            <div className="max-w-6xl mx-auto px-6 py-10">
                <Link href="/dashboard/test-series/company" className="inline-flex items-center gap-2 text-slate-500 hover:text-orange-600 font-medium mb-8 transition-all group px-5 py-2.5 rounded-full bg-white border border-slate-200 shadow-sm hover:shadow-md active:scale-95">
                    <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Back to Companies
                </Link>
                ...
                <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-10">
                    <div>
                        ...
                        <h1 className="text-5xl md:text-6xl font-black tracking-tight text-slate-900 mt-2">{displayLabel} MCQs</h1>
