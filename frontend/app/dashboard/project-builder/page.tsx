...
export default function ProjectBuilderWizard() {
    ...
    const domains = [
        ...
        { id: 'mobile', name: 'Mobile App', icon: Layers, color: 'text-slate-500', bg: 'bg-slate-50' },
                    ...
                    <AnimatePresence mode="wait">
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                                    {domains.map((domain) => (
                                        ...
                                        >
                                            ...
                                            <div>
                                                <h3 className={`font-bold text-lg ${selectedInterests.includes(domain.id) ? 'text-slate-900' : 'text-slate-900'}`}>
                                                    {domain.name}
                                                </h3>
                                                {selectedInterests.includes(domain.id) && (
                        ...
                        {step === 2 && (
                            ...
                            >
                                ...
                                <p className="text-slate-500 mb-6">Based on your interests in <span className="font-bold text-slate-800">{selectedInterests.map(id => domains.find(d => d.id === id)?.name).join(' + ')}</span></p>

                                <div className="grid grid-cols-1 gap-4 overflow-y-auto pr-2 custom-scrollbar mb-6">
                                    {[1, 2, 3].map((i) => (
                                        <div key={i} className="p-6 rounded-2xl border border-slate-100 bg-white hover:border-slate-200 hover:shadow-lg transition-all cursor-pointer group relative overflow-hidden">
                                            <div className="absolute top-0 right-0 p-3 opacity-10 group-hover:opacity-20 transition-opacity">
                                                <Sparkles size={64} className="text-slate-800" />
                                            </div>
                                            ...
                                            <div className="mt-4 pt-4 border-t border-slate-50 flex justify-end">
                                                <button
                                                    onClick={() => setStep(3)}
                                                    className="text-slate-800 font-bold text-sm flex items-center gap-1 hover:gap-2 transition-all"
                                                >
                            ...
                            >
                                ...
                                <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200 mb-8">
                                    <div className="flex justify-between items-start mb-4">
                                        <h2 className="text-3xl font-extrabold text-slate-900">AI Powered Resume Analyzer</h2>
                                        <span className="bg-white text-slate-800 px-4 py-1.5 rounded-full font-bold text-sm shadow-sm">
                                            Intermediate
                                        </span>
                                    </div>
                                    ...
                                    <div className="flex flex-wrap gap-2">
                                        {['React', 'Tailwind', 'Python', 'FastAPI', 'OpenAI API', 'LangChain'].map(tech => (
                                            <span key={tech} className="bg-white/60 text-slate-800 px-3 py-1 rounded-lg text-sm font-bold border border-slate-200">
                                                {tech}
                                            </span>
                                        ))}
                                    </div>
                                ...
                                <div className="mt-auto flex justify-end gap-4">
                                    ...
                                    <button
                                        onClick={() => setStep(4)}
                                        className="bg-slate-900 text-white px-8 py-3 rounded-xl font-bold flex items-center gap-2 hover:bg-slate-800 transition-all shadow-lg shadow-slate-200 hover:shadow-slate-200"
                                    >
                            ...
                            >
                                <div className="w-full space-y-6">
                                    <div className="text-center mb-8">
                                        <div className="w-16 h-16 bg-slate-100 rounded-2xl flex items-center justify-center mx-auto mb-4 text-slate-800">
                                            <Sparkles size={32} />
                                        </div>
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                            ...
                                            <div className="flex items-center gap-3 bg-white border border-slate-200 rounded-xl px-4 py-3 focus-within:ring-2 focus-within:ring-slate-200 focus-within:border-slate-300 transition-all">
                                        ...
                                        <div>
                                            ...
                                            <textarea placeholder="What was the biggest challenge you faced?" className="w-full bg-white border border-slate-200 rounded-xl px-4 py-3 focus:ring-2 focus:ring-slate-200 focus:border-slate-300 transition-all outline-none text-sm font-medium h-32 resize-none"></textarea>
                                        </div>
                                    </div>

                                    <button className="w-full bg-slate-800 text-white py-4 rounded-xl font-bold text-lg hover:bg-slate-900 transition-all shadow-xl shadow-slate-200 active:scale-[0.98]">
                                        Submit for Review
                                    </button>
