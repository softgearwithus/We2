...
export function generateStaticParams() {
  const companyPages = getAllSeoPages().filter(page => page.category === 'company');
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  if (!pageData || pageData.category !== 'company') {
  ...
  return {
    ...
    alternates: {
      canonical: `https://emble.in/company/${slug}`,
    },
    openGraph: {
      title: pageData.title,
      description: pageData.subDescription,
      url: `https://emble.in/company/${slug}`,
      siteName: 'Emble',
      images: [
        {
